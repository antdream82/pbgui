...
11. PB7 suite-result configs must be readable in PBGUI whether the suite data
    is stored in legacy nested form or current flattened form:
    - legacy: `backtest.suite.{enabled,include_base_scenario,base_label,aggregate,scenarios}`
    - current: `backtest.{suite_enabled,aggregate,scenarios}`
    This is required so `BT selected` can restore scenario editors from older
    and newer PB7 runs without silently dropping scenarios.

12. Backtest and optimize navigation must not reuse stale Streamlit suite keys
    after importing a result or draft config. Selection/import flows should
    clear prior suite widgets, reseed the new suite state, and drive tab changes
    through the pending rerun flag instead of mutating an instantiated widget.

13. Optimize EMA span sliders now allow the newer PB7 range used in research:
    - `Bounds.EMA_SPAN_0_MAX = 20000`
    - `Bounds.EMA_SPAN_1_MAX = 20000`
    This keeps current optimize configs loadable without Streamlit max-value
    errors.
